...
int list_devices() {
    ...
    while (snd_seq_query_next_client(client, cinfo) >= 0) {
        ...
        if (strcmp(client_name, DEFAULT_CLIENT_NAME) == 0)
            continue;
        ...
        while (snd_seq_query_next_port(client, pinfo) >= 0) {
            ...
            while ((bit = get_next_bit(&caps)) > 0)
                printf("%s ", get_capability(bit));

            unsigned int types = snd_seq_port_info_get_type(pinfo);

            if (types != 0) {
                printf("\n    types: ");
                while ((bit = get_next_bit(&types)) > 0)
                    printf("%s ", get_type(bit));
            }
